...
# output
out_dir <- 'figures/ab_fig_2_multi_samples_EVsign_grey'
...
# 自定义绘图函数，保证颜色分配合理
plot_ev_bar <- function(ev, label) {
  # 将NA替换为0，避免颜色异常
  ev[is.na(ev)] <- 0
  # 颜色分配：正数红，负数蓝，0灰
  ev_col <- ifelse(ev > 0, "red", ifelse(ev < 0, "blue", "gray"))
  # 确保颜色向量长度与数据长度一致
  if(length(ev_col) != length(ev)) {
    ev_col <- rep(ev_col, length.out = length(ev))
  }
...
for(chr in chroms){
  ...
  plot_ev_bar(EV.rbl[[chr]], "RBL")
  plot_ev_bar(EV.lcl[[chr]], "LCL")
  plot_ev_bar(EV.gcbc[[chr]], "GCBC")
  plot_ev_bar(EV.mbc[[chr]], "MBC")
  plot_ev_bar(EV.nbc[[chr]], "NBC")
  plot_ev_bar(EV.pc[[chr]], "PC")
